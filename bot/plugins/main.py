...
@Bot.on_message(filters.private & filters.command("status") & filters.user(ADMIN), group=-2)
async def sts(_, m: Message):
    total_users = await db.total_users_count()
    await m.reply_text(text=f"Total users is DB: {total_users}", quote=True)


@Bot.on_message(filters.private & filters.command("broadcast") & filters.user(ADMIN) & filters.reply, group=-1)
...
async def main(bot: Bot, message: Message):
    ...
    try:
        ...
        if not (user.status == "creator"
            ...
            return
        if user.can_delete_messages is not True:
